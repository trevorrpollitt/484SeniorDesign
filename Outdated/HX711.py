...
class HX711:
	...
	############################################################
	def _ready(self):
		if GPIO.input(self.CSID0) == 0:	# if DOUT pin is low data is ready for reading
			return True
		else:
			return False
    ############################################################
	...
	def _set_channel_gain(self, num):
			for i in range(num):
				...
				GPIO.output(self.CSID1, True) # set high
				GPIO.output(self.CSID1, False) # set low

...
long hx711_read(long tareValue);
long hx711_read(long tareValue)
{
    ...
    long longValue = 0; unsigned char index; long dataValue = 0;
...
void hx711_tare(void){
	...
	// pull an average value for tare
	for(i=0;i<100;i++){
		longValue = hx711_read(0);
		total+= longValue;
	}
...
int main(int argc, char *argv[ ])
{
    ...
    for(;;)
    {
        longValue2 = hx711_read(tareValue); //Read in value
		printf("%ld\n",longValue2);
		hx711_reset();
    }
